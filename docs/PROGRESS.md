...
* **Cierre de sesión:** el usuario se retira a probar la app por su cuenta y vuelve en otra sesión. Todo quedó commiteable y documentado (ver "Próximos pasos" abajo para retomar). No hay cambios a medio hacer ni archivos en estado roto — el repo está en un punto estable: smoke test pasa, notas de ejemplo limpias en `notes/`, `__pycache__` limpiado.
